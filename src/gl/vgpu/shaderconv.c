...
char * ConvertShaderVgpu(struct shader_s * shader_source){
    ...
    // For now, skip stuff
    if(!globalsvgpu.vgpu_force_conv && FindString(source, "#version 100")){
        if (globalsvgpu.vgpu_dump) {
...
char * ReplacePrecisionQualifiers(char * source, int * sourceLength){
    if (globalsvgpu.vgpu_precision != 0){
        char * target_precision = "";
        switch (globalsvgpu.vgpu_precision) {
    ...
    //int insertPosition = FindPositionAfterVersion(source);
    //source = InplaceReplaceByIndex(source, sourceLength, insertPosition, insertPosition-1, "\nprecision highp float;\n");
